...
In order to facilitate biological discovery
from transcriptomic regions located by Clip-Seq or other methods,
we have developed an automatic annotation system called
RNA Centric Annotation System (RCAS) that provides
...
The final report of RCAS consists of high-qulaity dynamic figures and tables,
...
which are located via transcriptomics method such as Clip-Seq.
...
Meanwhile user can also provide customized GFF/GTF file.

For this vignette.
to demonstrate RCAS functionality,
we  provide sample BED and GFF file via RCAS library,
which are accessible using a common R function, data().
However to import user-defined
BED and GFF/GTF files,
user should execute two RCAS functions called importBed() and importGtf()

## Using RCAS sample data
...
## Using RCAS functions
To use importBed() and importGtf(),
user should provide file paths to the respective
...
### Querying TxDB: Coverage Profile for 3' UTRs
