...
run_anot = FALSE #basic annotation summary is always run
...
```{r motif_results_figcap, echo=FALSE, results='asis', eval=run_motif}
...
if(length(motif$motifs) > 0){
  cat(paste0("**Figure-",figure_count,":** Discovered motifs"))
  par(mfrow=c(3,2), mar=c(2,2,2,2))
  for (i in 1:length(motif$motifs)){
    motif_pattern = motif$motifs[[i]]@pattern
    plotMotif(motif$motifs[[i]]@match$pattern, main=paste0('Motif-',i,': ',motif_pattern), cex=0.5)
  }
...
datatable(merged[1:500], filter = 'top')
