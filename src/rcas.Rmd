...
motifs = args[10] ##'../test/PARCLIP_AGO1234_Hafner2010a_hg19_xaa.anot-motif.tsv' #motif annotation output from top_motifs.py script of Dilmurat
figure_count = 1
```
...
```{r read_input_data, echo=FALSE, eval=TRUE}
mdata = fread(anot)
...
```{r MEME_top_motifs, results='asis', echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r MEME_motif_frequency, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
dt = motif_data
...
```{r MEME_top_genes, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
dt = motif_data
...
```{r MEME_gene_types, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
dt = motif_data
...
```{r MEME_gene_features, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
dt = motif_data
...
```{r query_gene_features, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r query_RNA_Genes, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r query_gene_types, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
dt = mdata
df = dt[,length(unique(peak_id)), by=gene_type]
colnames(df) = c("feature", "count")
df$percent = round(df$count / length(unique(dt$peak_id)) * 100, 1)
...
```{r chromosomes_gene_features,  echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r get_table_of_genes, echo=FALSE, eval=TRUE}
...
# Coverage Profiles
```{r coverage_profiles_read_input, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_transcripts, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_exons, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_exon-introns, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_introns, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_promoters, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_fiveUTRs, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r coverage_profiles_threeUTRs, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r GO_BP, results='asis', warning=FALSE, echo=FALSE, eval=TRUE}
go = fread(go_bp)
results = go[bonferroni < 0.001]
if (nrow(results) > 0){
  datatable(results, caption = 'Table 2: Significant Biological Process GO terms (FDR < 0.001) enriched for genes that overlap query regions',  filter = 'top')
}else{
...
```{r GO_MF, results='asis', warning=FALSE,  echo=FALSE, eval=TRUE}
go = fread(go_mf)
results = go[bonferroni < 0.001]
if (nrow(results) > 0){
  datatable(results, caption = 'Table 3: Significant Molecular Function GO terms (FDR < 0.001) enriched for genes that overlap query regions',  filter = 'top')
}else{
...
```{r GO_CC, results='asis', warning=FALSE,  echo=FALSE, eval=TRUE}
go = fread(go_cc)
results = go[bonferroni < 0.001]
if (nrow(results) > 0){
  datatable(results, caption = 'Table 4: Significant Cellular Compartment GO terms (FDR < 0.001) enriched for genes that overlap query regions',  filter = 'top')
}else{
...
```{r GSEA_msigdb, results='asis', warning=FALSE,  echo=FALSE, eval=TRUE}
if (msigdb == 'NOT_RUN'){
  cat ('**## WARNING: MSIGDB module was not run**','\n')
}else if(file.exists(msigdb)){ 
  msigdb = fread(msigdb)
  if (nrow(msigdb) > 0){
