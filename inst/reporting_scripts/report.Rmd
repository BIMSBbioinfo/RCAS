...
p1 <- plotly::plot_ly(data = cvgF, x = ~bases, y = ~meanCoverage, type = 'scatter', mode = 'lines', opacity = 0.6, showlegend = FALSE) %>% 
  add_ribbons(x = ~bases, ymin = cvgF$meanCoverage - cvgF$standardError*1.96, ymax = cvgF$meanCoverage + cvgF$standardError*1.96, opacity = 0.5) 

p2 <- plotly::plot_ly(data = cvgT, x = ~bases, y = ~meanCoverage, type = 'scatter', mode = 'lines', opacity = 0.6, showlegend = FALSE) %>% 
  add_ribbons(x = ~bases, ymin = cvgT$meanCoverage - cvgT$standardError*1.96, ymax = cvgT$meanCoverage + cvgT$standardError*1.96, opacity = 0.5) 
...
```{r cdsIntronCoverage_figcap, results='asis', eval = annotationSummary}
cat("## Coverage profile of query regions Coding Exon - Intron Boundaries\n")
cat("**Figure",figureCount,":** The depth of coverage of query regions at coding exon - intron junctions\n")
```

```{r cdsIntronCoverage, eval = annotationSummary}
cvg <- getFeatureBoundaryCoverageBin(queryRegions = queryRegions,
                                  featureCoords = txdbFeatures$cds,
                                  flankSize = 50,
                                  sampleN = 10000)

yLimit <- (as.integer(max(c(cvg$fivePrime, cvg$threePrime))/10)+1)*10

p <- plot_ly(x = cvg$bins, y = cvg$fivePrime, type = 'scatter', mode = 'lines') %>% 
  layout(p, 
        xaxis = list(showticklabels = F, title =  'Junction'), 
        yaxis = list(title = 'coverage', range = c(-10, yLimit)),
        showlegend = FALSE) 
plotly::add_annotations(p = p, 
                        bordercolor = "rgb(250,0,0)",
                        borderwidth = 3,
                        showarrow = F, 
                        text = c('< Intron (last 50 bp) >', '< Coding Exon (first 50 bp) >'), 
                        x = c(-25, 25), y = c(-10, -10))

p <- plot_ly(x = cvg$bins, y = cvg$threePrime, type = 'scatter', mode = 'lines') %>% 
  layout (p, 
          xaxis = list(showticklabels = F, title = 'Junction'),
          yaxis = list(title = 'coverage', range = c(-10, yLimit)), 
          showlegend = FALSE) 
plotly::add_annotations(p = p,
                        ...
                        borderwidth = 3,
                        showarrow = F,
                        text = c('< Coding Exon (last 50 bp) >', '< Intron (first 50 bp) >'),
                        x = c(-25, 25), y = c(-10, -10))

if(params$printProcessedTables == TRUE) {
  write.table(x = cvg, file=paste0('Figure',figureCount,'.cdsIntronCoverage.data.tsv'), quote = FALSE, sep = '\t', row.names = TRUE)
} 
...
p <- plotly::plot_ly(data = cvgList, type = 'scatter', mode = 'markers+lines')
for (f in unique(cvgList$feature)){
  data <- cvgList[cvgList$feature == f,]
  p <- add_trace(p = p, data = data, x = ~bins, y = ~meanCoverage, 
                 legendgroup = f, showlegend = FALSE, opacity = 0.4, color = f)
