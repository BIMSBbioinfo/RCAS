...
queryRegionsExt[width(queryRegionsExt) < 15] <- GenomicRanges::resize(x = queryRegionsExt[width(queryRegionsExt) < 15], 
                                                                      width = 15, 
                                                                      fix = 'center')
motifLogos <- getFeatureSpecificMotifLogos(queryRegions = queryRegionsExt, 
                                           txdbFeatures = txdbFeatures, 
...
```{r motif_analysis_plot, fig.width=10, fig.height=9, eval=motifAnalysis}
cowplot::plot_grid(plotlist = motifLogos, ncol = 2, scale = 0.9)
figureCount <- figureCount + 1
```
