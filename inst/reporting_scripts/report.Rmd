...
#split the coding-exons into corresponding transcripts
exons <- txdbFeatures$exons
exonCounts <- lengths(split(exons, names(exons)))
...
cvgF <- getFeatureBoundaryCoverage2(queryRegions = queryRegions, featureCoords = internalExons, boundaryType = 'fiveprime', sampleN = 10000)
cvgT <- getFeatureBoundaryCoverage2(queryRegions = queryRegions, featureCoords = internalExons, boundaryType = 'threeprime', sampleN = 10000)

p1 <- plotly::plot_ly(data = cvgF, x = ~bases, y = ~meanCoverage, type = 'scatter', mode = 'markers+lines', opacity = 0.6, showlegend = FALSE) %>% 
  add_ribbons(x = ~bases, ymin = cvgF$meanCoverage - cvgF$standardError*1.96, ymax = cvgF$meanCoverage + cvgF$standardError*1.96, opacity = 0.5) 

p2 <- plotly::plot_ly(data = cvgT, x = ~bases, y = ~meanCoverage, type = 'scatter', mode = 'markers+lines', opacity = 0.6, showlegend = FALSE) %>% 
  add_ribbons(x = ~bases, ymin = cvgT$meanCoverage - cvgT$standardError*1.96, ymax = cvgT$meanCoverage + cvgT$standardError*1.96, opacity = 0.5) 
