...
if (length(motifResults$motifs) > 0) {
  querySeqs <- extractSequences(queryRegions = queryRegions, 
                                genomeVersion = params$genomeVersion)
...
```{r GSEA_msigdb_tabcap, results='asis', eval=msigdbAnalysis}
...
```{r msigdb_analysis, eval=msigdbAnalysis}
if (params$species != 'human') {
  ...
  geneSets <- parseMsigdb(filePath = msigdbOutFile)
...
msigdbResults <- subset(msigdbResults, select = -c(BH, treatmentSize, fisherPVal))
...
if(params$printProcessedTables == TRUE) {
  write.table(x = msigdbResults, file=paste0('Table',tableCount,'.GSEA.data.tsv'), quote = FALSE, sep = '\t', row.names = TRUE)
} 
